...
\mnnote{TODO: Ajouter intro outils collaboratifs}
...
L'approche \ac{CRDT} offrant une meilleure capacité de passage à l'échelle et étant compatible avec une architecture \ac{P2P}, de nombreux travaux\cite{Nedelec2016CRATE, peerpad, serenity-notes} ont été entrepris pour proposer une alternative distribuée répondant à ces limites.
...
Notamment, elle développe \acf{MUTE}\footnote{Disponible à l'adresse : \url{https://mutehost.loria.fr}}\footnote{Code source disponible à l'adresse suivante : \url{https://github.com/coast-team/mute}}, un éditeur collaboratif \ac{P2P} temps réel chiffré de bout en bout.
...
\begin{figure}[!ht]
  \mnnote{TODO: Faire figure architecture système de MUTE}
...
\begin{figure}[!ht]
  \mnnote{TODO: Faire figure architecture logicielle de MUTE}
  ...
  \resizebox{\columnwidth}{!}{
    \begin{tikzpicture}
      \path
        ...
        ++(0:3) node[op, label=above:{$\langle A:1 \rangle$}] (a1) {a1}
        ++(0:3) node [op, label=above:{$\langle A:7 \rangle$}] (a7) {a7}
        ++(0:10) node[point, label=above:{$\langle A:7, B:3 \rangle$}, label=below:{\emph{deliver}}] (a-receives-b3) {};


      \draw[thick] (a0) --  (a1) -- (a7) -- (a-receives-b3) -- (a-end);

      \path
          ++(270:2) node {\textbf{B}}
          +(0:26) node (b-end) {}
          ++(0:0.5) node (b0) {}
          ++(0:6) node[point, label=above:{$\langle A:6 \rangle$}, label=below:{\emph{deliver}}] (b-receives-a1) {}
          ++(0:3) node[point, label=above:{$\langle A:7 \rangle$}, label=below:{\emph{deliver}}](b-receives-a7) {}
          ++(0:4) node[op, label=above:{$\langle B:3 \rangle$}] (b3) {b3}
          ++(0:9) node[point, label=above:{\emph{reply to sync query}}] (b-receives-c-sync-request) {};

      \draw[thick] (b0) -- (b-receives-a1) -- (b-receives-a7) -- (b3) -- (b-receives-c-sync-request) -- (b-end);

      \path
          ++(270:4) node {\textbf{C}}
          +(0:26) node (c-end) {}
          ++(0:0.5) node (c0) {}
          ++(0:6) node (c-receives-a1) {}
          +(90:0.5) node[cross] (fail-a1) {}
          ++(0:3) node (c-receives-a7) {}
          +(90:0.5) node[cross] (fail-a7) {}
          ++(0:7) node[point, label=below:{$\langle A:7,B:3 \rangle$}, label=above:{\emph{deliver}}] (c-receives-b3) {}
          ++(0:3) node[op, label=below:{$\langle B:3 \rangle$}] (c-query-sync) {sync}
          ++(0:6) node[point, label=below:{$\langle A:7,B:3 \rangle$}, label=above:{\emph{deliver}}] (c-receives-b-sync-reply) {};


      \draw[thick] (c0) -- (c-receives-b3) -- (c-query-sync) --  (c-receives-b-sync-reply) -- (c-end);

      \draw[->, dashed, shorten >= 1]
        (a1.east) edge (b-receives-a1.west)
        (a1.east) edge (fail-a1)
        (a7.east) edge (b-receives-a7.west)
        (a7.east) edge (fail-a7)
        (b3.east) edge (a-receives-b3.west)
        (b3.east) edge (c-receives-b3.west)
...
\mnnote{TODO: Choisir vecteur de version initial des noeuds et MàJ figure et texte en accordance.}
...
\begin{figure}[!ht]
  \mnnote{TODO: Compléter figure webrtc}
...
\begin{figure}[!ht]
  \mnnote{TODO: Compléter figure trusternity}
