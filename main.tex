...
En effet, cette valeur correspond à l'index de $\trm{id}$ dans l'\emph{ancien état} (\ie $\trm{renamedIds[offset]} = \trm{id}$, \eg $\trm{renamedIds[0]} =$ \id{i}{B0}{0}).
...
Dans cette situation, $\trm{newId}$ a été obtenu en utilisant \textsc{renIdFromPredId} et nous avons $\trm{id} = \trm{tail}$ (\eg $\trm{id} = $ \id{i}{B0}{0}\id{m}{B1}{0}).
...
Ainsi, pour préserver l'ordre voulu, \textsc{revertRenameId} renvoie $\trm{id}$ with $\trm{id} = \trm{predOfSuccId~\top~tail}$.
