...
\begin{itemize}
  ...
  \item Lorsque modifications possibles sur un type de données sont commutatives, l'intégration des modifications effectuées par les autres noeuds ne nécessite aucun mécanisme particulier, même dans le cas où le noeud courant a effectué ses propres modifications en parallèle.
  ...
  \item Ainsi, ce qui définit un CRDT est sa sémantique en cas de concurrence et son modèle de synchronisation.
    ...
    Puis nous présenterons les différents modèles de synchronisation proposés dans la littérature, et détaillerons leurs contraintes et impact sur les \ac{CRDT} les adoptant.
\end{itemize}
