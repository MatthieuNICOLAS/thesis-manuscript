  ...
  \resizebox{\columnwidth}{!}{
    \begin{tikzpicture}
        \path
            node {\textbf{A}}
            to ++(0:0.5 * \widthletter) node[epoch] {\epoch{0}}
            to ++(0:1.05 * \widthoriginepoch) node[letter, label=below:{\id{i}{B0}{0}}] {A}
            to ++(0:\widthletter) node[letter, label=below:{\id{l}{C0}{0}}] {B}
            to ++(0:\widthletter) node[letter, label=below:{\id{t}{A0}{0}}] {C}
            to ++(0:\widthletter) node[letter, label=below:{\id{y}{D0}{0}}] (S0A-right) {D}
            ...
            to ++(0:5 * \widthletter) node[epoch] (S2A-left) {\epoch{A1}}
            to ++(0:1.3 * \widthepoch) node[block, fill=mydarkblue,
                    label={below:{\color{mydarkblueid}\id{i}{A1}{0..1}} }
                        ] {AB}
            to ++(0:\widthblock) node[letter, fill=mylightblue,
        ...
        \path
            ...
            to ++(0:\widthletter) node[letter, label=below:{\id{l}{C0}{0}}] {B}
            to ++(0:\widthletter) node[letter, label=below:{\id{t}{A0}{0}}] {C}
            to ++(0:\widthletter) node[letter, label=below:{\id{y}{D0}{0}}] (S0B-right) {D}

            to ++(0:21 * \widthletter) node (S1B) {?};

        \path
          to ++(270:1)
          to ++ (0:14 * \widthletter) node[cross] (fail) {};
  ...
  \resizebox{\columnwidth}{!}{
    \begin{tikzpicture}
        \path
            ...
            to ++(0:5 * \widthletter) node[epoch] (S1A-left) {\epoch{0}}
            to ++(0:1.05 * \widthoriginepoch) node[letter, label=below:{\id{i}{B0}{0}}] {A}
            to ++(0:\widthletter) node[letter, label=below:{\id{l}{C0}{0}}] {B}
            to ++(0:\widthletter) node[letter, fill=mydarkorange,
                    label=below:{\color{mydarkorange}\id{m}{A1}{0}}] {X}
            to ++(0:\widthletter) node[letter, label=below:{\id{t}{A0}{0}}] {C}
            to ++(0:\widthletter) node[letter, label=below:{\id{y}{D0}{0}}] (S1A-right) {D}

            to ++(0:5 * \widthletter) node[epoch] (S2A-left) {\epoch{A2}}
            to ++(0:1.3 * \widthepoch) node[block, fill=mydarkblue,
        ...
        \path
            ...
            to ++(0:21 * \widthletter) node[epoch] (S1B-left) {\epoch{A2}}
            to ++(0:1.3 * \widthepoch) node[block, fill=mydarkblue,
                    label={below:{\color{mydarkblueid}\id{i}{A1}{0..1}} }
                        ] {AB}
            to ++(0:\widthblock) node[block, fill=mydarkblue,
                    label={below:{\color{mydarkblueid}\id{i}{A1}{3..4}} }
                        ] (S1B-right) {CD}

            to ++(0:5 * \widthletter) node[epoch] (S2B-left) {\epoch{A2}}
            to ++(0:1.3 * \widthepoch) node[block, fill=mydarkblue,
                    label={below:{\color{mydarkblueid}\id{i}{A1}{0..4}} }
                        ] (S2B-right) {ABXCD};
          ...
          \resizebox{\columnwidth}{!}{
            \begin{tikzpicture}
              \path
                  node {\textbf{A}}
                  to ++(0:0.5 * \widthletter) node[epoch] {\epoch{0}}
                  to ++(0:1.05 * \widthoriginepoch) node[letter, label=below:{\id{i}{A0}{0}}] {A}
                  to ++(0:\widthletter) node[letter, label=below:{\id{l}{B0}{0}}] {B}
                  to ++(0:\widthletter) node[letter, label=below:{\id{t}{A1}{0}}] {C}
                  to ++(0:\widthletter) node[letter, label=below:{\id{y}{B1}{0}}] (S0A-right) {D}

                  to ++(0:5 * \widthletter) node[epoch] (S1A-left) {\epoch{A2}}
                  to ++(0:1.3 * \widthepoch) node[block, fill=mydarkblue,
              ...
              \path
                  ...
                  to ++(0:\widthletter) node[letter, label=below:{\id{l}{B0}{0}}] {B}
                  to ++(0:\widthletter) node[letter, label=below:{\id{t}{A1}{0}}] {C}
                  to ++(0:\widthletter) node[letter, label=below:{\id{y}{B1}{0}}] (S0B-right) {D}
                  ...
                  to ++(0:7 * \widthletter) node[epoch] (S3B-left) {\epoch{B2}}
                  to ++(0:1.3 * \widthepoch) node[block, fill=mydarkpurple,
                          label={above:{\color{mydarkpurpleid}\id{i}{B2}{0..1}} }
                              ] {AB}
                  to ++(0:\widthblock) node[letter,
                          label={below:{\id{i}{B2}{1}\id{l}{B0}{0}\id{f}{A3}{0}} }
                              ] {X}
                  to ++(0:\widthletter) node[block, fill=mydarkpurple,
  ...
  \subfloat[Arbre des époques de B à la réception de l'opération \emph{insert}]{
    \begin{minipage}{\linewidth}
        ...
        \begin{tikzpicture}[scale=0.8,every node/.style={scale=0.8}]
            \path
                node[op] (e0) {\epoch{0}}
                to ++(225:2) node[op] (eA1) {\epoch{A2}};
            \path
                to ++(315:2) node[op, red] (eB2) {\epoch{B2}};
          ...
          \resizebox{\columnwidth}{!}{
            \begin{tikzpicture}
              \path
                  ...
                  to ++(0:3.7 * \widthletter) node[epoch] (S3A-left) {\epoch{B2}}
                  to ++(0:1.3 * \widthepoch) node[block, fill=mydarkpurple,
                          label={above:{\color{mydarkpurpleid}\id{i}{B2}{0..1}} }
                              ] {AB}
                  to ++(0:\widthblock) node[letter,
  ...
  \subfloat[Arbre des époques de A avant la réception de l'opération \emph{rename} vers l'époque \epoch{B2}]{
    \begin{minipage}{0.5 \linewidth}
        ...
        \begin{tikzpicture}[scale=0.8,every node/.style={scale=0.8}]
            \path
                node[op] (e0) {\epoch{0}}
                to ++(270:1.8) node[op, red] (eA2) {\epoch{A2}};
...
Ces complexités dépendent de plusieurs paramètres : nombre d'identifiants et de blocs stockés au sein de la structure, taille des identifiants, structure de données utilisée...
...
Finalement, nous considérons qu'un arbre AVL est utilisé comme structure de données pour représenter l'état interne de la séquence et que des tableaux sont utilisés pour représenter les \emph{anciens états}.

\mnnote{
  TODO: Trouver structure de données adaptée à l'\emph{arbre des époques}.
  Besoin de pouvoir établir rapidement le chemin entre la racine et une époque.
  Pour cela, le mieux serait d'avoir accès directement à l'époque et qu'elle référencie l'époque parente.
}

\mnnote{
  NOTE: Une table de hachage correspond bien (ce que j'utilise dans l'implem).
  Mais pas le plus adapté pour la garbage collection des époques obsolètes (besoin de parcourir l'ensemble des clés et de supprimer celles n'appartenant plus aux \emph{époques requises}).
}

\subsubsection{Complexité en temps des opérations \emph{insert} et \emph{remove}}

À partir de ces hypothèses, nous établissons les complexités des opérations.
...
De manière similaire à \textsc{revertRenameId}, \textsc{renameId} genère pour un identifiant donné un nouvel identifiant équivalent à l'époque suivante en se basant sur son prédecesseur.
...
\begin{itemize}
  ...
  \item On obtient dans ce cas une complexité de O($h$ + $n$ ($k$ + $l$ $\cdot$ log($n$) + log($b$)))
  ...
  \item Permet de ramener la complexité du renommage des $n$ identifiants de l'époque \ac{LCA} à l'époque courante à O($l$ ($n$ + $n$))
  ...
  \item Peut donc obtenir une complexité de O($h$ + $k$ $\cdot$ $n$ + $l$ ($n$ + $n$) + $n$ + $b$) pour l'intégration de l'opération \emph{rename} distante
\end{itemize}
...
\begin{table}[!ht]
  ...
  \begin{tabular}{lcc}
      ...
      \emph{naive rename} & $n$ & $h + n (k + l \cdot \log{}n + \log{}b)$ \\
      \emph{rename} & $n$ & $b + n (k + 2 \cdot l + 1) + b$ \\
      \bottomrule
  \end{tabular}
...
La seconde étape consiste à parcourir l'arbre des époques de manière inverse à l'ordre définit par la relation \emph{priority}.
Ce parcours se poursuit jusqu'à trouver l'époque maximale causalement stable, \ie la première époque pour laquelle l'opération \emph{rename} associée est causalement stable.
...
À partir de ces chemins, le mécanisme calcule le \ac{LCA}.
Pour ce faire, l'algorithme calcule de manière successive la dernière intersection entre le chemin de la racine jusqu'au \ac{LCA} courant et les chemins précédemment calculés.
Le \ac{LCA} est la dernière époque du chemin résultant.
...
Pour chaque chemin, il ajoute les époques se trouvant après le \ac{LCA} à l'ensemble des \emph{époques requises}.
...
\begin{table}[!ht]
  ...
  \resizebox{0.7\columnwidth}{!}{
      \begin{tabular}{lrrrrr}
          ...
          Type & Nb Ops (k) &   Mean &   Median & 99\textsuperscript{ème} Quant. &    Std \\
          \midrule
...
Afin de comparer les performances de RenamableLogootSplit et de LogootSplit de manière globale, nous avons mesuré le temps nécessaire pour un nouveau noeud pour rejouer l'entièreté du log d'opérations d'une session de collaboration.
Nous présentons les résultats obtenus dans \autoref{fig:time-to-replay-log}.

\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.7\columnwidth]{img/replay-log-30k-with-legend.png}
  \caption{Évolution du temps nécessaire pour rejouer le log d'opérations}
  \label{fig:time-to-replay-log}
\end{figure}

Nous observons que le gain sur le temps d'intégration des opérations \emph{insert} et \emph{remove} permet initialement de contrebalancer le surcoût lié aux opérations \emph{rename}.
...
Cette tendance est davantage prononcé dans les scénarios avec des opérations \emph{rename} concurrentes.
Dans un cas réel d'utilisation, ce scénario (\ie rejouer l'entièreté du log) ne correspondra au scénario principal et pourra être mitigé, par exemple en utilisant un mécanisme de compression du log d'opérations.
Dans la \autoref{sec:rename-as-compression-mechanism}, nous présentons comment mettre en place un tel mécanisme en se basant justement sur l'opération \emph{rename}.
...
Les résultats de ce benchmark sont présentés dans \autoref{tab:impact-frequency}.
...
\begin{table}[!ht]
  ...
  \resizebox{.8\linewidth}{!}{
    \begin{tabular}{llrrrrrrrrrr}
      \toprule
      \multicolumn{2}{c}{Paramètres} & \multicolumn{5}{c}{Temps d'intégration ($\trm{\mu}$s)}  & \multicolumn{5}{c}{Taille (B)} \\
      \cmidrule(lr){1-2} \cmidrule(lr){3-7} \cmidrule(lr){8-12}
      Type & CRDT & Moyenne & Médiane &  IQR & 1\textsuperscript{er} Quant. & 99\textsuperscript{ème} Quant. & Moyenne & Médiane &  IQR & 1\textsuperscript{er} Quant. & 99\textsuperscript{ème} Quant. \\
      \midrule
      ...
      \midrule
      \multicolumn{2}{c}{Paramètres} & \multicolumn{5}{c}{Temps d'intégration (ms)}  & \multicolumn{5}{c}{Taille (KB)} \\
      \cmidrule(lr){1-2} \cmidrule(lr){3-7} \cmidrule(lr){8-12}
      Type & CRDT & Moyenne & Médiane &  IQR & 1\textsuperscript{er} Quant. & 99\textsuperscript{ème} Quant. & Moyenne & Médiane &  IQR & 1\textsuperscript{er} Quant. & 99\textsuperscript{ème} Quant. \\
      \midrule
...
La valeur de cette borne est définit par la fréquence de l'opération \emph{rename}.
...
Nous avons décrit précédemment dans \autoref{sec:integration-process-rename-op}, et plus précisément dans \autoref{alg:renameRemote}, le processus d'intégration de l'opération \emph{rename} évaluée dans ce manuscrit.
...
Dans ces travaux, les auteurs définissent l'opération \emph{rebalance} qui permet aux noeudds de réassigner des identifiants plus courts aux éléments du document.
...
Ce nouveau type de données répliquées associe à LogootSplit un mécanisme de renommage optimiste permettant de réduire ponctuellement les métadonnées stockées et d'optimiser l'état interne de la structure de données.
...
En cas d'opérations \emph{rename} concurrentes, la relation d'ordre total stricte \lepoch permet aux noeuds de décider quelle opération \emph{rename} utiliser, sans coordination.
...
\mnnote{
  TODO: Voir pour une autre implémentation.
  Implémentation actuelle dûe à la séparation entre \texttt{DocService} et \texttt{Sync} : \texttt{Sync} n'a pas de moyen de connaître l'époque courante.
  Donc peut pas cibler et ajouter uniquement le dot de l'opération \emph{rename} correspondante.
}
...
\begin{itemize}
  ...
  \item Serait intéressant de comparer à d'autres méthodes de synchronisation : mécanisme d'anti-entropie basé sur un Merkle Tree \mnnote{TODO: retrouver ref}, synchronisation par états (state/delta-based \acp{CRDT}).
\end{itemize}
...
\begin{itemize}
  ...
  \item Computationnellement lourd.
    \mnnote{TODO: Limite provenant d'une slide de JP, voir ce qui justifie cette remarque}
