...
\begin{definition}[LogootSplit-tuple]
  Un \emph{LogootSplit-tuple} t est un quadruplet <position, node\_id, node\_seq, offset> où
  \begin{itemize}
    \item position incarne la position souhaitée de l'élément.
    \item node\_id est l'identifiant unique du noeud qui a généré t.
    \item node\_seq est le numéro de séquence courant du noeud à la génération de t.
...
Dans ce manuscrit, nous représentons les tuples par le biais de la notation suivante : \id{position}{node\_id~node\_seq}{offset} où $\trm{position}$ est une lettre minuscule, $\trm{node\_id}$ une lettre majuscule et $\trm{node\_seq}$ et $\trm{offset}$ des entiers, \eg \id{i}{B0}{0}.

À partir de là, les identifiants LogootSplit sont définis de la manière suivante :

\begin{definition}[LogootSplit-id]
  Un \emph{LogootSplit-id} i est une liste de \emph{LogootSplit-tuple}.
\end{definition}

Nous représentons les identifiants en listant les tuples qui les composent.
Par exemple, l'identifiant composé des tuples \emph{<i, B, 0, 0><f, A, 0, 0>} est présenté de la manière suivante : \id{i}{B0}{0}\id{f}{A0}{0}.
...
\begin{definition}[Relation $<_{id}$]
  ...
  \begin{enumerate}[label={(\roman*)}]
    \item les $i^{emes}$ premiers tuples de $id_1$ et $id_2$ sont égaux, les $j^{emes}$ premiers éléments de leur $i+1$ tuples respectifs sont égaux et le $j+1^{eme}$ élément du $i+1$ tuple de $id_1$ est inférieur au $j+1^{eme}$ du $i+1$ tuple de $id_2$ (\eg \id{i}{B0}{0} $<_{id}$ \id{m}{A0}{0}, \id{i}{A1}{0} $<_{id}$ \id{i}{B0}{0}, \id{i}{A1}{0}\id{f}{A1}{0} $<_{id}$ \id{i}{A1}{0}\id{m}{B1}{0})
...
\mnnote{TODO: indiquer que le couple (node\_id), node\_seq) permet d'identifier de manière unique la base d'un bloc ou d'un identifiant}

Notons que pour une séquence donnée, nous pouvons identifier chacun de ses identifiants par le triplet <node\_id, node\_seq, offset> issue de leur dernier Id-tuple.
Par exemple, le triplet <B, 0, 2> désigne de manière unique l'identifiant \id{i}{B0}{2} dans \autoref{fig:logootsplit-seq}.
