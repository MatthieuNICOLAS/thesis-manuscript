...
\begin{definition}[Tuple]
  ...
  \begin{itemize}
    ...
    \item nodeId est l'identifiant unique du noeud qui a généré t.
    \item nodeSeq est le numéro de séquence courant du noeud à la génération de t.
...
Pour ce faire, une relation d'ordre totale aux identifiants basée sur l'ordre lexicographique est associée à l'ensemble des identifiants :

\begin{definition}[Relation $<_{id}$]
  Soient deux LogootSplit-id $id_1$ et $id_2$, la relation $id_1 <_{id} id_2$ est définie si et seulement si
  \begin{enumerate}[label={(\roman*)}]
    \item les $i^{emes}$ premiers composants de $id_1$ et $id_2$ sont égaux et le $i+1^{eme}$ composant de $id_1$ est inférieur au $i+1^{eme}$ composant de $id_2$ (\eg \id{i}{B0}{0} $<_{id}$ \id{m}{A0}{0}, \id{i}{A1}{0} $<_{id}$ \id{i}{B0}{0}, \id{i}{A1}{0}\id{f}{A1}{0} $<_{id}$ \id{i}{A1}{0}\id{f}{A1}{1})
    \item la liste de tuples composant $id_1$ est un préfixe de la liste de tuples composant $id_2$ (\eg \id{i}{B0}{0} $<_{id}$ \id{i}{B0}{0}\id{f}{A0}{0})
  \end{enumerate}
\end{definition}
...
\begin{definition}[IdInterval]
  Un \emph{IdInterval} est un couple (idBegin, offsetEnd) où
  \begin{itemize}
...
Nous représentons l'interval d'identifiants qui composent un bloc à l'aide du formalisme suivant : \id{position}{nodeId~nodeSeq}{begin..end} où $\trm{begin}$ est l'offset du premier identifiant du bloc et $\trm{end}$ du dernier.
...
\mnnote{TODO: indiquer que le couple (nodeId, nodeSeq) permet d'identifier de manière unique la base d'un bloc ou d'un identifiant}

Notons que pour une séquence donnée, nous pouvons identifier chacun de ses identifiants par le triplet $\langle$nodeId, nodeSeq, offset$\rangle$ issue de leur dernier Id-tuple.
...
\begin{itemize}
  \item Structure de données définie de la manière suivante :
\end{itemize}
...
\begin{definition}[Séquence LogootSplit]
  ...
  \begin{itemize}
    ...
    \item blocks est une liste de couples (idInterval, elts) correspondant à l'état actuel de la séquence répliquée.
  \end{itemize}
\end{definition}

\begin{itemize}
  \item Plusieurs fonctions disponibles pour permettre de modifier localement la séquence
  \item Ces fonctions génèrent des opérations à diffuser aux autres noeuds pour qu'ils mettent à jour leur état
  \item insert(index, elts) qui génère une opération \emph{insert} correspondante
  \item remove(index, length) qui génère une opéraiton \emph{remove} correspondante
\end{itemize}

\begin{definition}[insert]
  Une opération \emph{insert} est un couple (id, elts) où
  \begin{itemize}
...
\mnnote{TODO: Ajouter et dérouler exemple où des noeuds insèrent dans une séquence répliquée pour illustrer la façon de choisir position, d'append à un bloc, ou de split un bloc}
...
Notons que l'époque générée est caractérisée et identifiée de manière unique par son couple (nodeId, nodeSeq).
...
Il est donc nécessaire de faire évoluer le modèle de livraison des opérations pour ajouter la contrainte suivante : toute opération doit désormais être livrée après l'opération \emph{rename} qui a introduit son époque de génération.
...
\begin{table}[!ht]
  ...
  \begin{tabular}{lcc}
      ...
      \cmidrule(lr){2-3}
        & Locale &   Distante \\
      \midrule
      \emph{insert} & log($n$) & $k$ + $l$ $\cdot$ log($m$) + log($n$) \\
      \emph{remove} & log($n$) & $k$ + $l$ $\cdot$ log($m$) + log($n$) \\
      \emph{rename} & $n$ & $k \cdot n$ + $l$ $\cdot$ ($n$ + $m$) \\
      \bottomrule
  \end{tabular}
  \caption*{$n$ : nombre de blocs, $k$ : nombre d'opérations \emph{rename} à inverser, $l$ : nombre d'opérations \emph{rename} à appliquer, $m$ : nombre de blocs de l'\emph{ancien état} de l'opération \emph{rename} à appliquer}
  %}
\end{table}

\mnnote{TODO: J'ai pas pris en compte le fait que le renommage peut me décomposer le bloc à insérer/supprimer en plusieurs. Modifier les complexités en conséquence.}
\mnnote{TODO: J'ai pas pris en compte le fait qu'un remove peut contenir plusieurs blocs à supprimer. Modifier les complexités en conséquence.}
\mnnote{TODO: J'ai pas pris en compte le fait qu'il faut recréer une séquence (\ie insérer les blocs une fois renommés) lors de l'intégration de l'opération rename distante}
...
\mnnote{TODO: Ajouter la complexité temporelle de cette implémentation alternative}
