...
\begin{itemize}
  \item Pour limiter la coordination entre les noeuds, systèmes distribués adoptent le paradigme de la réplication optimiste.
    Paradigme consiste à ce que chaque noeud possède une copie de la donnée.
    ...
    Mécanisme de synchronisation leur permet ensuite de partager leurs modifications respectives et de nouveau converger.
    Ce paradigme offre aux noeuds une haute disponibilité \mnnote{TODO: insérer ref sticky model} et une faible latence.
  \item Afin d'ordonner les modifications effectués dans un système, la littérature repose généralement sur la relation \emph{happen-before} \cite{1978-happen-before-lamport}.
    Nous l'adaptons ci-dessous à notre contexte :
    \begin{definition}[Happen-before]
      La relation \emph{happen-before} indique qu'une modification $m_1$ a eu lieu avant une modification $m_2$, notée $m_1 \rightarrow m_2$, si et seulement si une des conditions suivantes est respectée :
      ...
      \end{enumerate}
    \end{definition}
    Dans le cadre d'un système distribué, on note que la relation \emph{happen-before} ne permet pas d'établir un ordre total entre les opérations.
    ...
    \end{definition}
  \item Lorsque modifications possibles sur un type de données sont commutatives, l'intégration des modifications effectuées par les autres noeuds, même concurrentes, ne nécessite aucun mécanisme particulier.
    Cependant, majorité des types de données proposent des opérations de modifications qui ne sont pas commutatives car de sémantiques contraires.
    Ainsi, une exécution dans un système distribué suivant le paradigme de réplication optimiste peut mener à la génération d'opérations concurrentes non commutatives.
    Parle alors de conflits.
    La figure \autoref{fig:set-conflict} présente un scénario où des opérations de sémantiques opposées sont générées en concurrence.
    ...
    \begin{figure}[!ht]
      ...
      \resizebox{\columnwidth}{!}{
        \begin{tikzpicture}
          \path
              ...
              +(0:7) node[point, label=above right:{\{\}}, label=below left:{\emph{rmv(a)}}] (a-removes) {}
              +(0:12) node[point, label=above right:{\{a\}}, label=below left:{\emph{add(a)}}] (a-add) {}
          ...
          \path
              ...
              +(0:15.5) node[point, label=below right:{\{\}}, label=above left:{\emph{rmv(a)}}] (b-removes) {}
      ...
      \caption{Exécution distribuée présentant un conflit provoqué par l'ajout et la suppression d'un même élément en concurrence}
      \label{fig:set-conflict}
    \end{figure}

  \item Dans cet exemple, deux noeuds A et B répliquent et partagent un même Ensemble.
  ...
  \item À l'issue de ce scénario, l'état à produire n'est pas trivial : le noeud A a exprimé son intention d'ajouter l'élément $a$ à l'ensemble, tandis que le noeud B a exprimé son intention contraire de retirer l'élément $a$ de ce même ensemble.
    Nous pouvons juger tout aussi correct les états $\{a\}$ et $\{\}$ dans cette situation.
    Afin de permettre aux noeuds de continuer leur collaboration, il est cependant primordial qu'ils convergent vers le même état.
    Pour ce faire, nécessité d'un mécanisme de résolution de conflits, potentiellement automatique.
  \item Les \acp{CRDT} répondent à ce besoin.
    S'agit de nouvelles spécifications des types de données existants, \eg Ensemble ou Séquence.
    Conçus pour être utilisés dans des systèmes distribués adoptant la réplication optimiste.
    Offrent les deux propriétés suivantes :
    \begin{enumerate}[label=(\roman*)]
      \item Peuvent être modifiés sans coordination avec les autres noeuds.
      \item Tous noeuds ayant observé le même ensemble de modifications obtiendront des états équivalents.
        S'agit de la propriété de cohérence forte à terme.
    \end{enumerate}
  \item Pour ce faire, la spécification des \acp{CRDT} reposent sur la théorie des treillis :
    \begin{enumerate}[label=(\roman*)]
      \item États forment un sup-demi-treillis, possédant une relation d'ordre partiel $\leq$.
      \item Modifications génèrent par inflation un nouvel état supérieur ou égal à l'état original d'après $\leq$.
    ...
    \end{enumerate}
  \item Malgré leur spécification différente, les \acp{CRDT} partagent la même sémantique et interface que les types non-répliqués correspondant du point de vue des utilisateurs.
  ...
  \item Plusieurs sémantiques valides peuvent être proposées pour ces nouveaux scénarios.
    Un \ac{CRDT} se doit donc de préciser quelle sémantique il choisit.
  \item L'autre aspect définissant un \ac{CRDT} donné est le modèle qu'il adopte pour propager les modifications.
  ...
  \item Ainsi, ce qui définit un CRDT est sa sémantique en cas de concurrence et son modèle de synchronisation.
...
\begin{itemize}
  \item Plusieurs sémantiques peuvent être proposées pour résoudre les conflits.
  \item Sémantique simple est de trancher entre l'ensemble des opérations en conflit et d'en sélectionner une de manière arbitraire.
    \mnnote{TODO: Préciser que le but est de recréer un ordre total sur les opérations s'approchant de celui instauré par une horloge globale.}
    Cette sémantique, dite du Last-Writer Wins (LWW) \mnnote{TODO: citer papier}, est une sémantique générique que l'on retrouve dans une variété de \acp{CRDT} proposés pour des types différents.
    Ainsi, avec cette sémantique, utilisons une horloge pour déterminer quelle opération choisir.
    L'horloge physique étant sujette à des imprécisions, a généralement recours à une horloge logique \cite{1978-happen-before-lamport} \mnnote{TODO: Ajouter refs des horloges logiques plus intelligentes (Interval Tree Clock, Hybrid Clock...)}.
  \item D'autres sémantiques sont spécifiques au type de donnée concerné.
    Dans le cadre de l'Ensemble, on remarque qu'un conflit est produit lorsque des modifications \emph{add} et \emph{remove} sont effectuées en concurrence.
    Deux sémantiques peuvent alors être proposées pour le résoudre :
    \begin{enumerate}[label=(\roman*)]
      \item Une sémantique où l'opération \emph{add} d'une valeur a la précédence sur les opérations concurrentes \emph{remove} de la même valeur, nommée Add-wins.
      \item Une sémantique où l'opération \emph{remove} d'une valeur a la précédence sur les opérations concurrentes \emph{add} de la même valeur, nommée Remove-wins.
    \end{enumerate}
  ...
  \item Ainsi, pour un même type de données, différents \ac{CRDT} peuvent être spécifiés, chacun proposant la sémantique qu'il utilise pour résoudre les conflits.
    ...
    \mnnote{TODO: Ajouter use-case d'un CL set.}
\end{itemize}

\mnnote{NOTE:
  Pourrait ajouter quelques lignes sur le fait que la sémantique peut même mener à un changement du domaine du \ac{CRDT} par rapport au domaine du type séquentiel.
  Exemple du MV-Register CRDT dont les états sont des ensembles, et non plus simplement une valeur comme pour le type séquentiel ou encore le LWW-Register CRDT.
}
...
\begin{itemize}
  ...
  \item Finalement, il est intéressant de noter que cette approche met en place par nature une diffusion des modifications suivant le modèle de cohérence causale \mnnote{TODO: Ajouter ref}.
