...
\begin{figure}[!ht]
  ...
  \begin{algorithmic}[5]
      \Function{renRemote}{seq, epochTree, currentEpoch, renOp}
          ...
          \\
          \State addEpoch(epochTree, introducedEpoch, opEpoch)
          \\
          \If{introducedEpoch \lepoch currentEpoch}
            \State \Return $\langle$seq, currentEpoch$\rangle$
          \Else
            \State idIntervals $\gets$ getIdIntervals(seq)
            \State ids $\gets$ flatMap(idIntervals, expand)
            \\
            ...
            \\
            \For{epoch in epochsToRevert}
              \State renamedIds $\gets$ getRenamedIds(epochTree, epoch) \label{alg:renameRemote-retrieve-rename-data-1}
              \State nId $\gets$ getNodeId(epochTree, epoch)
              \State nSeq $\gets$ getNodeSeq(epochTree, epoch) \label{alg:renameRemote-retrieve-rename-data-2}
              \State revertRenameId\textsubscript{partial} $\gets$ papply(revertRenameId, renamedIds, nId, nSeq)
              \State ids $\gets$ map(ids, revertRenameId\textsubscript{partial}) \label{alg:renameRemote-revertRenameId}
            \EndFor
            \\
            \For{epoch in epochsToApply} \label{alg:renameRemote-renameId-1}
              \State renamedIds $\gets$ getRenamedIds(epochTree, epoch)
              \State nId $\gets$ getNodeId(epochTree, epoch)
              \State nSeq $\gets$ getNodeSeq(epochTree, epoch)
              \State renameId\textsubscript{partial} $\gets$ papply(renameId, renamedIds, nId, nSeq)
              \State ids $\gets$ map(ids, renameId\textsubscript{partial})
            \EndFor \label{alg:renameRemote-renameId-2}
            \\
            \State nId $\gets$ getNodeId(seq)
            \State nSeq $\gets$ getNodeSeq(seq)
            \State newIdIntervals $\gets$ aggregate(ids) \label{alg:renameRemote-aggregate}
            \State content $\gets$ getContent(seq)
            \State \Return $\langle$\new~LogootSplit(nId, nSeq, newIdIntervals, content), introducedEpoch$\rangle$
        \EndIf
      \EndFunction
  \end{algorithmic}
  \caption{TODO}
...
\begin{itemize}
  ...
  \item Principale limite est que nécessite par contre de maintenir et de parcourir périodiquement le log des opérations pour répondre aux requêtes de synchronisation.
    ...
    Peut pas tronquer les opérations du log car nécessaire pour mettre à niveau les nouveaux pairs (à moins d'un mécanisme de snapshot comme évoqué dans \autoref{sec:rename-as-snapshot}).
