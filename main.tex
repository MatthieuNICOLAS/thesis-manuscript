...
\begin{itemize}
  ...
  \item remove(index, length) qui génère une opération \emph{remove} correspondante
\end{itemize}
  ...
  \resizebox{\columnwidth}{!}{
    \begin{tikzpicture}
        ...
        \draw[dashed, ->, thick, shorten >= 3]
          (S1A-right.east) edge node[right, align=center]{\emph{remove} \id{i}{B0}{1}}  (S2B-left.west)
...
\begin{itemize}
  \item Figure décrit scénario suivant
  \item Deux noeuds, A et B, répliquent et éditent collaborativement un document texte.
  \item Ils partagent initialement le même état, composé d'un seul bloc associant les identifiants \id{i}{B0}{0..3} aux éléments "HRLO".
  \item Puis les noeuds se mettent à éditer le document.
  \item Le noeud A commence par supprimer l'élément "r".
  \item LogootSplit génère l'opération \emph{remove} correspondante à diffuser aux autres noeuds de la collaboration en utilisant l'identifiant de l'élément supprimé (\id{i}{B0}{1}).
  \item Le noeud A insère ensuite un élément "e" dans la séquence, entre le "h" et le "l"
  \item LogootSplit doit alors générer un identifiant $id$ à associer à ce nouvel élément
  \item Ce nouvel identifiant $id$ doit respecter la contrainte suivante : \id{i}{B0}{0} \lid $id$ \lid \id{i}{B0}{2}
  \item Le noeud A ne peut pas générer un identifiant composé d'un seul tuple respectant cet ordre
  \item Le noeud A génère alors $id$ en recopiant le premier tuple (\id{i}{B0}{0}) et en y ajoutant un nouveau tuple (\id{f}{A0}{0})
  \item LogootSplit génère l'opération \emph{insert} correspondante
  \item En parallèle, le noeud B insère un élément "!" à la fin de la séquence
  \item Comme le noeud B est l'auteur du bloc \id{i}{B0}{0..3}, il peut y append ou prepend de nouveaux éléments (en supposant qu'il s'agisse bien des bornes minimales et maximales du bloc qui n'aient jamais existées)
  \item L'identifiant \id{i}{B0}{4} est donc associé à l'élément "!" et celui-ci est ajouté au bloc existant
  \item Les noeuds se synchronisent ensuite
  \item Le noeud A reçoit l'opération \emph{insert} de l'élément "!" à la position \id{i}{B0}{4} issue de B
  \item Grâce à l'identifiant, le noeud A est capable d'insérer cet élément à l'emplacement voulu
  \item De son côté, le noeud B reçoit tout d'abord l'opération \emph{remove}
\end{itemize}
...
\begin{itemize}
  \item Livraison exactly-once des opérations
  \item Une opération \emph{remove} doit être livrée après l'opération \emph{insert} correspondante
  \item Des opérations \emph{insert} peuvent elles arriver dans des ordres arbitraires
  \begin{itemize}
    \item Même celles causalement liées
  \end{itemize}
\end{itemize}
