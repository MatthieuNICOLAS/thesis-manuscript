...
\begin{figure}[!ht]
  ...
  \begin{algorithmic}
      \Function{insRemote}{seq, epochTree, currentEpoch, insOp}
          \If{currentEpoch $\neq$ opEpoch }
            \State insertedIdInterval $\gets$ getInsertedIdInterval(insOp)
            \State ids $\gets$ expand(insertedIdInterval)
            \\
            \State opEpoch $\gets$ getEpoch(insOp)
            \State $\langle$epochsToRevert, epochsToApply$\rangle$ $\gets$ getPathBetweenEpochs(epochTree, opEpoch, currentEpoch)
            \\
            ...
            \State insertOps $\gets$ generateInsertOps(newIdIntervals, content)
            \For{insertOp in insertOps}
              \State execute(seq, insertOp)
            \EndFor
          \Else
            \State execute(seq, insertOp)
        \EndIf
