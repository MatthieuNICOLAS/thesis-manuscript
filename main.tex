...
\autoref{alg:renameId} présente seulement le cas principal de \textsc{renameId}, \ie le cas où l'identifiant à renommer appartient à l'intervalle des identifiants formant l'ancien état ($\trm{firstId} \leq_{id} \trm{id} \leq_{id} \trm{lastId}$).
Les fonctions pour gérer les autres cas, \ie les cas où l'identifiant à renommer n'appartient pas à cet intervalle ($\trm{id} <_{id} \trm{firstId}$ ou $\trm{lastId} <_{id} \trm{id}$), sont présentées dans \autoref{app:rename-id}.
...
Cependant, la fonction \textsc{renameId} décrite dans \autoref{alg:renameId} permet seulement aux noeuds de progresser d'une époque \emph{parente} à une de ses époques \emph{enfants}.
...
\begin{figure}[!ht]
  ...
  \caption{Annuler une opération \emph{rename} intégrée précèdemment}
...
Nous décrivons cette fonction dans \autoref{alg:revertRenameId}.
...
Dans ce cas, $\trm{newId}$ peut résulter d'une opération \emph{insert} concurrent à l'opération \emph{rename} (\ie Cas \ref{item:revert-rename-2}).
Nous avons alors $\trm{newId} \in ]\trm{newPredId~predId}, \trm{newPredId~succId}[$.
...
$\trm{newId}$ ne peut avoir été inséré que causalement après l'opération \emph{rename} (\ie \autoref{item:revert-rename-3}).
Nous avons alors $\trm{newId} \in ]\trm{newPredId}, \trm{newPredId~predId}[$.
Puisque $\trm{newId}$ a été inséré causalement après l'opération \emph{rename}, il n'existe pas de constrainte sur la valeur à retourner autre que la \autoref{prop:order}.
Pour gérer ce cas, nous introduisons deux nouveaux tuples exclusifs au mécanisme de renommage : $\trm{MIN\_TUPLE}$ et $\trm{MAX\_TUPLE}$.
Ils sont respectivement le tuple minimal et maximal utilisables pour générer des identifiants.
En utilisant $\trm{MIN\_TUPLE}$, \textsc{revertRenameId} est capable de renvoyer une valeur pour $\trm{id}$ adaptée à l'ordre voulu (avec $\trm{id} = \trm{predId~MIN\_TUPLE~tail}$).

Le dernier sous-cas est le pendant du sous-cas précédent et se produit lorsque nous avons $\trm{succId} < \trm{tail}$.
Nous avons alors $\trm{newId} \in ]\trm{newPredId~succId}, \trm{newSuccId}[$.
La stratégie pour gérer ce cas est similaire et consiste à ajouter un préfixe pour créer l'ordre voulu.
Pour générer ce préfixe, \textsc{revertRenameId} utilise  $\trm{predOfSuccId}$ and $MAX\_TUPLE$.
$\trm{predOfSuccId}$ est obtenue en décrémentant le dernier $\trm{offset}$ de $\trm{succId}$.
Ainsi, pour préserver l'ordre voulu, \textsc{revertRenameId} renvoie $\trm{id}$ with $\trm{id} = \trm{predOfSuccId~MAX\_TUPLE~tail}$.
...
La seconde étape nécessite elle de parcourir l'arbre AVL et de convertir chaque idInterval en Identifiers, ce qui nécessite \bigO{n} instructions.
...
Les concepteurs de systèmes doivent proposer un compromis entre les différents paramètres en fonction des constraintes du système concerné (application temps réel ou asynchrone, limitations matérielles des noeuds...).
          ...
          \resizebox{.7\linewidth}{!}{
            \begin{tikzpicture}
              \path
                ...
                to ++(0:0.5) node (a0) {}
                to ++(0:1) node[point, label=above:{rename to \epoch{A1}}] (a1) {}
                to ++(0:2) node (a-receives-b7) {}
                to ++(0:1) node [point, label=above:{rename to \epoch{A10}}] (a10) {}
                to ++(0:1) node (a11) {}
                to ++(0:1) node (a99) {}
                to ++(0:1) node[point, label=above:{rename to \epoch{A100}}] (a100) {}
                to ++(0:1.5) node (a-receives-c1) {}
                to ++(0:1.5) node (a-end) {};
              ...
              \path
                  to ++(270:2) node {\textbf{B}}
                  to ++(0:0.5) node (b0) {}
                  to ++(0:2) node (b-receives-a1) {}
                  to ++(0:0.5) node[point, label=below:{rename to \epoch{B2}}] (b7) {}
                  to ++(0:2.5) node (b8) {}
                  to ++(0:1) node (b99) {}
                  to ++(0:2.5) node (b-receives-a100) {}
                  to ++(0:1.5) node (b-end) {};
              ...
              \path
                  to ++(270:4) node {\textbf{C}}
                  to ++(0:0.5) node (c0) {}
                  to ++(0:2) node[point, label=below:{rename to \epoch{C1}}] (c1) {}
                  to ++(0:3) node (c2) {}
                  to ++(0:1) node (c99) {}
                  to ++(0:4) node (c-end) {};
          ...
          \resizebox{.15\linewidth}{!}{
            \begin{tikzpicture}
                \path
                    node[op] (e0) {\epoch{0}}
                    to ++(225:1.5) node[op] (eA1) {\epoch{A1}}
                    to ++(270:1.5) node[op] (eB7) {\epoch{B7}}
                    to ++(270:1.5) node[op] (eA10) {\epoch{A10}}
                    to ++(270:1) node (eA11) {}
                    to ++(270:0.5) node (eA99) {}
                    to ++(270:1) node[op] (eA100) {\epoch{A100}};
                \path
                    to ++(315:1.5) node[op, red] (eC1) {\epoch{C1}};
