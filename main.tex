...
\begin{itemize}
  ...
  \item Afin d'ordonner les modifications effectués dans un système, la littérature repose généralement sur la relation \emph{happen-before} \cite{1978-happen-before-lamport}.
    ...
    \begin{definition}[Happen-before]
      La relation \emph{happen-before}, indique qu'une modification $m_1$ a eu lieu avant une modification $m_2$, noté $m_1 \rightarrow m_2$, si et seulement si une des conditions suivantes est respectée :
      \begin{enumerate}[label=(\roman*)]
        \item $m_1$ a eu lieu avant $m_2$ sur le même noeud.
        \item $m_1$ a été délivrée au noeud auteur de $m_2$ avant la génération de cette dernière.
  ...
  \item Lorsque modifications possibles sur un type de données sont commutatives, l'intégration des modifications effectuées par les autres noeuds, même concurrentes, ne nécessite aucun mécanisme particulier.
    ...
    \mnnote{TODO: Exemple avec Ensemble}
    Pour résoudre ces situations et permettre aux noeuds de converger, nécessité d'un mécanisme de résolution de conflits.
  \item Les \acp{CRDT} répondent à ce besoin.
    ...
    \end{itemize}
  \item De la perspective des utilisateurs, les \acp{CRDT} partagent la même sémantique et interface que les types non-répliqués correspondant.
    Mais leur particuliarité est d'être conçus pour supporter nativement les exécutions concurrentes.
    Les \acp{CRDT} partagent donc le comportement des types non-répliqués dans le cadre d'exécutions séquentielles, mais définissent aussi une nouvelle sémantique pour chaque nouveau scénario ne pouvant se produire que dans le cadre d'une exécution distribuée.
...
\begin{itemize}
  ...
  \item Dernière sémantique proposée récemment : celle du CL-Set.
    ...
    \begin{enumerate}[label=(\roman*)]
      \item \emph{add} et \emph{remove} d'un élément prennent place à tour de rôle, chaque modification écrasant la précédente.
    ...
    \end{enumerate}
    À partir de ces observations, propose de simplement suivre pour chaque élément la chaîne d'ajouts et retraits la plus longue pour déterminer si l'élément est présent ou non dans l'ensemble.
  \item Ainsi, pour un même type de données, différents \ac{CRDT} peuvent être spécifiés, chacun proposant la sémantique qu'il utilise pour résoudre les conflits.
    Il est à noter qu'aucune sémantique n'est intrasèquement meilleure que les autres.
    Il revient alors aux concepteur-rices d'applications de choisir les bons \ac{CRDT} en fonction des besoins et des comportements attendus en cas de conflits.
...
\begin{itemize}
  \item Types de données répliquées sans conflits à base d'opérations
  \begin{itemize}
  ...
  \end{itemize}
  \item Types de données répliquées sans conflits à base de différences
\end{itemize}
...
\mnnote{TODO: Récapituler sous la forme d'un tableau les spécificités, avantages et inconvénients de chaque famille de CRDTs}
