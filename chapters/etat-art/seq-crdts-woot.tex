...
Par exemple, l'insertion de l'élément "E" dans la séquence "SA" pour obtenir l'état "SEA", \ie $\trm{ins}(1, E)$, devient $\trm{ins}(S \prec E \prec A)$, où $\prec$ représente l'ordre créé entre ces éléments.
...
Par exemple, considérons une séquence WOOT représentant "MR" et qui associe respectivement les identifiants $A1$ et $A2$ aux éléments "S" et "A".
L'insertion de l'élément "E" dans cette séquence pour obtenir l'état "SEA", \ie $\trm{ins}(S \prec E \prec A)$, produit par exemple l'opération $\trm{ins}(A1 \prec \langle B1,E \rangle \prec A2)$.
De manière similaire, la suppression de l'élément "E" dans cette séquence pour obtenir l'état "SA", \ie $\trm{rmv}(1)$, produit $\trm{rmv}(B1)$.
...
En effet, la relation $\prec$ n'est pas définie pour deux éléments insérés en concurrence et qui possèdent les mêmes prédecesseur et successeur, \eg $\trm{ins}(S \prec E \prec A)$ et $\trm{ins}(S \prec K \prec A)$.
