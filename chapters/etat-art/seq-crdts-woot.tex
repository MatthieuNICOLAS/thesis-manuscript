  ...
  \resizebox{\columnwidth}{!}{
    \begin{tikzpicture}
      ...
      \path
          ...
          +(0:12) node[point, label=-170:{$\trm{ins}(E \prec L \prec M)$}, label={[xshift=45pt]-10:{$\trm{ins(A2 \prec \langle A5,L \rangle \prec A1)}$}}] (a-ins-l) {}
...
\begin{definition}[Modèle de livraison WOOT]
  ...
  \begin{enumerate}[label=(\roman*)]
    \item Une opération doit être livrée au moins une fois à chaque noeud\footnotemark.
      \footnotetext{Pour être précis, l'algorithme décrivant l'intégration de l'opération $\trm{ins}$ dans \cite{2006-woot-oster} impose une livraison en exactement un exemplaire de l'opération.
      Il peut cependant être amélioré pour devenir idempotent, et donc permettre une livraison répétée de la même opération, en vérifiant au préalable la présence de l'identifiant dans la séquence.}
