Ce premier exemple a pour but d'illustrer la nécessité de la propriété de livraison en \emph{exactement un exemplaire} des opérations.
Pour cela, considérons l'exemple représenté par la \autoref{fig:why-exactly-once-delivery}.
  ...
  \resizebox{\columnwidth}{!}{
    \begin{tikzpicture}
      ...
      \path
        ...
        +(0:7) node[point, label=-170:{$\trm{ins}(B \prec X \prec C)$}, label={[xshift=6em]-10:{$\trm{ins}({\color{ucl1orange}\betterid{p}{A1}{2}\betterid{m}{A2}{1}},X)$}}] (a-ins-x) {}
        +(0:17) node[point] (a-recv-rmv-x) {}
        +(0:24) node (a-final) {};
      ...
      \path
        ...
        +(0:9) node[point] (b-recv-ins-x) {}
        +(0:15) node[point, label=170:{$\trm{rmv}(X)$}, label={[xshift=1em]10:{$\trm{rmv}({\color{ucl1orange}\betterid{p}{A1}{2}\betterid{m}{A2}{1}})$}}] (b-rmv-x) {}
        +(0:22) node[point] (b-recv-ins-x-2) {}
        +(0:24) node (b-final) {};
