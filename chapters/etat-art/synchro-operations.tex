...
Ce modèle de livraison permet de respecter le modèle de cohérence causale et ainsi de simplifier raisonnement sur les exécutions.
...
Les opérations mises en attente ne sont livrées qu'une fois l'opération perdue re-diffusée et livrée.
...
Pour résumer, cette approche permet de mettre en place un système en composant un \ac{CRDT} synchronisé par opérations avec un intergiciel de diffusion et de livraison.
...
Une partie de la complexité de cette approche réside ainsi dans l'ajustement du couple $\langle \trm{CRDT}, \trm{intergiciel} \rangle$ pour régler finement et optimiser leur fonctionnement en tandem.
Des approches \cite{baquero2017pure, 2021-improving-reactivity-pure-op-based-crdts-bauwens} ont proposées un patron de conception pour ces deux composants et leurs interactions, mais reposent encore sur une livraison causale des opérations.
