...
\begin{enumerate}
  \item $\trm{nodeId}$, l'identifiant du noeud qui génère l'identifiant.
    Il est supposé unique.
  \item $\trm{nodeSeq}$, un entier propre au noeud, servant d'horloge logique.
    Il est incrémenté à chaque génération d'identifiant.
\end{enumerate}
...
Pour ordonner un identifiant par rapport à ceux générés en concurrence, l'approche à pierres tombales peut définir une relation d'ordre total strict sur leur paire $\langle \trm{nodeId}, \trm{nodeSeq} \rangle$ respective, \eg en se basant sur l'ordre lexicographique.
