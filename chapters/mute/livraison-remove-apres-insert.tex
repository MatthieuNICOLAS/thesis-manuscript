          ...
          \resizebox{\columnwidth}{!}{
            \begin{tikzpicture}
              ...
              \draw[dashed, ->, thick, shorten >= 3]
                ...
                (S2B-right.east) edge node[below right, align=center]{$\trm{remove}$ {\color{mydarkorange}\id{i}{B0}{1..1}}} (S2A-left.west)
                (S2B-right.east) edge node[pos=0.80, right, align=center]{$\trm{remove}$ {\color{mydarkorange}\id{i}{B0}{1..1}}} (S1C.west);
...
Cet exemple reprend et complète celui de la \autoref{fig:why-causal-remove}.
...
Comme indiqué précédemment, l'opération \emph{b1} étant une opération $\trm{remove}$, le module de livraison calcule ses dépendances avant de la diffuser.
