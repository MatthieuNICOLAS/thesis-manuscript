...
Le noeud A effectue une première opération $\trm{rename}$ (\emph{a1}) puis une seconde opération $\trm{rename}$ (\emph{a7}), et les diffuse.
En concurrence, le noeud B génère et propage sa propre opération $\trm{rename}$ (\emph{b3}).
De son côté, le noeud C reçoit les opérations \emph{b3}, puis \emph{a1} et \emph{a7}.
Il émet ensuite une opération $\trm{insert}$ (\emph{c1}).
