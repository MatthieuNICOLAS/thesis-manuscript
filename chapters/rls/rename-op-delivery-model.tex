...
Dans la \autoref{fig:rls-invalid-delivery-order}, les noeuds A et B répliquent tous deux une même séquence, contenant les éléments "WOLD".
Tout d'abord, le noeud A procède au renommage de cet état.
Puis il insère un nouvel élément, "R", entre "O" et "L".
...
Dans cet exemple, les noeuds A et B répliquent tous deux une même séquence, contenant les éléments "WOLD".
Le noeud A commence par insérer un nouvel élément, "R", entre les éléments "O" et "L".
...
Comme l'époque de génération de l'opération $\trm{insert}$ (\epoch{0}) est différente de celle de son état courant (\epoch{A3}), le noeud B utilise \texttt{renameId} pour renommer l'identifiant avant de l'insérer.
