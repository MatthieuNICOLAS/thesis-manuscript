...
Toutes les 200 $\pm$ 50ms, chaque bot génère localement une opération $\trm{insert}$ ou $\trm{remove}$ et la diffuse immédiatement aux autres noeuds.
...
Chaque bot doit générer 15k opérations $\trm{insert}$ ou $\trm{remove}$, et s'arrête donc une fois qu'il a intégré les 150k opérations.
...
Les \emph{renaming bots} génèrent des opérations $\trm{rename}$ toutes les 7.5k ou toutes les 30k opérations qu'ils observent, en fonction des paramètres de la simulation.
