...
\footnotetext{
    ...
    Finalement, nous désignons par $\_$ une ou un ensembe de valeurs non-nécessaires par notre algorithme.
...
Ce premier cas correspond à l'algorithme que nous avons décrit de manière informelle lors de la présentation de l'opération \emph{rename} \cf{sec:rename-op-proposition}.
...
\begin{enumerate}
    ...
    Ce dernier peut être récupéré via une recherche de l'identifiant donné dans l'ancien état\footnote{Notons que l'ancien état est un tableau trié d'identifiants, il est donc possible d'effectuer une recherche dichotomique pour trouver efficacement l'index d'un identifiant.} (ligne \ref{alg:renameId-id-in-renamedids-begin}).
\end{enumerate}
...
Cet algorithme recherche le prédecesseur de l'identifiant donné (\id{i}{B1}{0}\id{m}{B2}{0}) dans l'ancien état, \ie l'identifiant maximal présent dans l'ancien état et qui est inférieur à l'identifiant donné (ligne \ref{alg:renameId-find-predecessor}).
