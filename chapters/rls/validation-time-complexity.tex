...
Ainsi, en reprenant l'ensemble des étapes composant l'intégration de l'opération $\trm{insert}$ distante, nous obtenons la complexité suivante : \bigO{h + s (k + l \cdot \log{}n + \log{}b)}.
...
Étudions à présent la complexité en temps de l'opération $\trm{rename}$.
...
L'intégration de l'opération $\trm{rename}$ se décompose en les étapes suivantes :
\begin{enumerate}
...
Nous obtenons donc une complexité en temps de \bigO{h + n(k + l \cdot \log{}n + \log{}b)} pour l'intégration de l'opération $\trm{rename}$ distante.
...
Ainsi, ces améliorations nous permettent d'obtenir une complexité en temps en \bigO{h + n (k + l) + b} pour le traitement de l'opération $\trm{rename}$ distante.
...
\begin{table}[!ht]
  ...
  \begin{tabular}{lcc}
      ...
      \cmidrule(lr){2-3}
        & Locale &   Distante \\
      \midrule
      $\trm{insert}$ & $\log{}b$ & $h + s (k + l \cdot \log{}n + \log{}b)$ \\
      $\trm{remove}$ & $\log{}b$ & $h + s (k + l \cdot \log{}n + \log{}b)$ \\
      \emph{naive rename} & $b$ & $h + n (k + l \cdot \log{}n + \log{}b)$ \\
      $\trm{rename}$ & $b$ & $h + n (k + l) + b$ \\
      \bottomrule
  \end{tabular}
